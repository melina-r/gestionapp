...
export default function Credits() {
  ...
  const fetchCredits = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`${baseUrl}/expenses/credits/${currentUserId}`);
  ...
  const filtered = credits
    .filter(c => {
      ...
      if (fecha) {
        if (!isSameDay(parseISO(c.creado_en), fecha)) return false;
      }
      ...
      <main className="main-content">
        ...
        <div className="table-wrapper">
          <table className="details-table">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} style={{textAlign:"center"}}>🔄 Cargando créditos...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={6} style={{textAlign:"center"}}>📭 Sin créditos</td></tr>
              ) : (
                filtered.map(c => (
